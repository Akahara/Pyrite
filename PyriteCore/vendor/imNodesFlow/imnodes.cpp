...
{
    if (start_type != ImNodesAttributeType_Input && start_type != ImNodesAttributeType_Output) return {};
...
int GetContextHoveredPinId()
{
    if (!GImNodes->HoveredPinIdx.HasValue()) return -1;
    return GImNodes->HoveredPinIdx.Value();
}
